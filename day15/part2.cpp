...
int main()
{
  ...
  for(int i = 0; i < max; i++){
    ...
    int toBeCheckedY = i;
    for(auto &info: relevantInfos){
      if(info->baconPos[1] == toBeCheckedY && info->baconPos[0] < max){
      ...
      if(info->sensorPos[1] + info->maxDistance >= toBeCheckedY && info->sensorPos[1] - info->maxDistance <= toBeCheckedY){
        ...
        int pos1Max = info->sensorPos[0] + availableXSteps > max ? max : info->sensorPos[0] + availableXSteps;
        int pos2Min = info->sensorPos[0] - availableXSteps > 0 ? info->sensorPos[0] - availableXSteps : 0;

        if(pos2Min < max && pos1Max > 0){
      ...
      if(coveredSpots == max){
        if(spotsCoveredRanges[1][1] - spotsCoveredRanges[0][0] == 2){
          cout << "{" << spotsCoveredRanges[0][0] + 1  << ", " << toBeCheckedY << "}" <<  " -> " << (spotsCoveredRanges[0][0] + 1) * (long long)max + toBeCheckedY << endl;
        } else {
          cout << "{" << spotsCoveredRanges[1][0] + 1  << ", " << toBeCheckedY << "}" << (spotsCoveredRanges[1][0] + 1) * (long long)max + toBeCheckedY  <<  endl;
        }
      }
  }
}