...
class Monkey{
	public:
	// create constructor
	Monkey(vector<int> items, array<string, 2> operation, array<int, 3> test, vector<Monkey*> *monkeyFriends){
		this->items = items;
	...
	int startOp(int inp){
		if(operation[1] == "old"){
			...
			case '+':
				return floor((inp * 2) / 3);
				break;
			case '*':
				return floor((pow(inp, 2)) / 3);
				break;
			}
		} else {
			switch(operation[0].at(0)){
				case '+':
					return floor((stoi(operation[1]) + inp) / 3);
					break;
				case '*':
					return floor((stoi(operation[1]) * inp) / 3);
					break;
...
int main()
{
	...
	std::sort(inspects.begin(), inspects.end(), greater());
	write(inspects[0] * inspects[1]);
}