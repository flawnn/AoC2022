...
int calculateShortestDist(Node* startingNode, int turn){
	string ascii_lowercase = "abcdefghijklmnopqrstuvwxyzE";
	Node* winningNode;	
    deque<Node*> frontier;

	if(turn >= (startingNode->move).size()){
		(startingNode->move).resize(turn+1);
		startingNode->move[turn] = 0;
	}
		...
		if(ii != 0){
			...
			if((row + ii >= 0 && row + ii < maze.size())){
				...
				if(next->move[turn] == -1){
					...
					next->parent = last;
					next->move[turn] = last->move[turn] + 1;
			...
			if((column + ii >= 0 && column + ii < maze[row].size())){
				...
				if(turn >= (next->move).size()){
					(next->move).resize(turn+1);
					next->move[turn] = -1;
				}
...
int main()
{
  ...
  while(getline(cin, input)){
	...
	int ii = 0;
	distances.push_back(calculateShortestDist(startingNodes[0], ii));
