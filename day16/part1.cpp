...
pair<vector<pair<Node*, int>>, int> calcBestPath(pair<vector<pair<Node*, int>>, int> currentCombo, int minutesLeft){
  pair<Node*, int> lastStep = (currentCombo.first.back());
  pair<vector<pair<Node*, int>>, int> bestCombo = currentCombo;
...
if(distance + 1 > minutesLeft || currentCombo.first.end() != find_if(currentCombo.first.begin(), currentCombo.first.end(), 
      ...
      pair<vector<pair<Node*, int>>, int> nextSteps = calcBestPath({nextCombo, currentCombo.second + potential}, minutesLeft - (distance + 1));

      if(nextSteps.second > bestCombo.second){
        bestCombo = nextSteps;
      }
    }
  } else {
...
int main()
{
  ...
  pair<vector<pair<Node*, int>>, int> bestPath = calcBestPath({{{startingNode, 0}}, 0}, 30);
  
  cout << bestPath.second << endl;
}
